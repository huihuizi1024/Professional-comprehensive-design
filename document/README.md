...
### 1. Web 管理平台 (`sourcecode/web`)
面向管理员的后台管理系统。
- **快递柜管理**：查看柜体状态、格口使用情况、新增/编辑柜机。
- **用户管理**：管理普通用户和快递员账号，审核快递员资质。
- **数据统计**：可视化展示系统运行数据。

### 2. 用户端小程序 (`sourcecode/user_xcx`)
面向普通用户的使用端。
- **查找柜机**：查看附近快递柜及空闲格口。
- **取件服务**：输入取件码或扫码一键取件。
- **寄件服务**：在线下单寄件，预约格口。
...
### 3. 快递员端小程序 (`sourcecode/app_server`)
面向快递员的作业端。
- **投递服务**：扫描/选择柜体，快速投递包裹。
- **格口管理**：查看格口状态，控制柜门开启。
- **订单管理**：查看已投递订单状态。
...
### 小程序 (`sourcecode/user_xcx` / `sourcecode/app_server`)
- **平台**：微信小程序原生开发
...
│   └── start.js                # 一键启动脚本
└── document/                   # 文档（本文档、API、数据库说明、设计/需求文件等）
```
...
我们在 `sourcecode/start.js` 提供了一个自动化脚本，可以一键准备环境并启动所有服务。

```bash
# 在项目根目录下执行
node sourcecode/start.js
```
...
- **用户端**：导入 `sourcecode/user_xcx` 目录。
- **快递员端**：导入 `sourcecode/app_server` 目录。
...
1.  **启动数据库**：
    ...
    docker run --name express-mysql \
      ...
      -p 3306:3306 \
      -v "$(pwd)/sourcecode/server/database/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" \
      -d mysql:8.0
    ```

2.  **启动后端**：
    ```bash
    cd sourcecode/server/backend
